<template>
  <div class="container">
    ...
    <div class="col2">
      <img
        src="https://res.cloudinary.com/dwslf2mo2/image/upload/v1578937276/zpxand58rzaajg36dci9.jpg"
        alt=""
      />
    </div>
  </div>
  ...
  methods: {
    ...
    addTouchHandler: function() {
      ...
      gesuredZone.addEventListener(
        ...
        event => {
          let touchX =
            (event.targetTouches[0].screenX - window.innerWidth / 2) /
            window.innerWidth /
            2;
          let touchY =
            (event.targetTouches[0].screenY - window.innerHeight / 2) /
            window.innerHeight /
            2;
          this.camera.position.x = Math.sin(touchX * Math.PI) * 1000;
          this.camera.position.y = -Math.sin(touchY * Math.PI) * 1000;
          this.camera.lookAt(new THREE.Vector3(0, 0, 0));
          this.renderer.render(this.scene, this.camera);
        },
        false
